...
from division import models as division_models
...
def add_fees_structure(request):
    ...
    fees_title_list = fees_models.Fees.objects.filter(school_id=school_id).values('id','title')
    class_list = class_models.school_class_mapping.objects.filter(school_id=school_id).values_list('class_id','class_id__class_name')
    division_list = division_models.Division.objects.filter(school_id=school_id).values('id','division_name')

    action_id_list=[]
    bank_deatils=[]
    jsonlist=[]
    class_data=[]
    #print("class_dataclass_data",class_list)
    for i in class_list:
        case2={'id':i[0],'name':i[1]}
    ...
    if request.method == 'POST':
        ...
        division_name = request.POST.getlist('division')
        fees_amt = request.POST.getlist('fees_amt')
        fees_id = request.POST.getlist('fees_id')
        ...
        count=0
        for i in class_name:
            class_obj = class_models.class_master.objects.get(id=i)
            for j in division_name:
                division_obj = division_models.Division.objects.get(id=j)
                for k in fees_id:
                    fees_obj = fees_models.Fees.objects.get(id=k)
                    fees_allocation_obj = fees_models.Fees_allocation.objects.create(fees_id=fees_obj,amount=fees_amt[count],class_id=class_obj,division_id=division_obj,due_date=due_date)
                    fees_allocation_obj.save()
        ...
        return render(request, 'add_fees_structure.html', {'fees_title_list':fees_title_list,'class_data':class_data,'division_list':division_list})


@login_required
@csrf_exempt
def get_manage_fees_report(request):
    data = []
    user_type = ""
    district = ""
    state = ""
    ...
    fees_info = fees_models.Fees_allocation.objects.filter(fees_id__school_id=school_id[0][0]).values_list('id','fees_id__title', 'fees_id__description', 'fees_id__posted_by__user__first_name','amount', 'division_id__division_name','class_id__class_name','fees_id__created_at','fees_id__fees_transaction__transaction_amount').order_by(
        '-id')
    ...
    for i in fees_info:
        ...
        class_name = i[6]
        posted_date = i[7]
        payment_received = i[8]
        toatal_count = '5'
        count += 1



        data.append([count, str(title),str(description), str(class_name), str(division_name), str(amount),str(payment_received),str(toatal_count),str(posted_by),str(posted_date.strftime("%d/%m/%Y")),
                     "<a href='/fees_report/view_fees_structure/" + str(
                         allocation_id) + "' class='btn'><i class='fas fa-eye'></i> View</a>"])
#print("data",data)
    return render(request, 'get_fees_allocation.html', {'data': (data)})
...
def edit_fees(request, fees_id_pk):
    ...
    if fees_obj is  not None:
        data=[str(fees_obj.description),str(fees_obj.bank_id.id),str(fees_obj.bank_id.account_number)]
    else:
        data=[]
...
@csrf_exempt
def view_fees_structure(request, fees_id_pk):
    print('_______view_fees_structure________')
    user_id = request.user.id
    school_id = user_models.UserProfile.objects.get(user=user_id)

    bank_list = bank_models.Bank.objects.filter(school_id=school_id.school_id).values('id', 'bank_name', 'account_number')

    try:
        fees_obj = fees_models.Fees_allocation.objects.get(id=fees_id_pk)
    except:
        fees_obj = None
    if fees_obj is not None:
        data = [str(fees_obj.fees_id.description), str(fees_obj.fees_id.bank_id.bank_name), str(fees_obj.fees_id.bank_id.account_number)]
    else:
        data = []

    return render(request, 'view_fees_report.html',
                  {'data': data, 'fees_id_pk': fees_id_pk, 'bank_list': bank_list})
def send_file(request):

    file_path = "C:/Users/Downloads/feesreport_master.xlsx" #Enter your path for master files here
